"""
Barras agrupadas — 7 métricas finales por modelo.
...
if missing:
    raise KeyError(
        f"Columnas no encontradas en los resultados: {missing}\n"
        "Vuelve a ejecutar la evaluación con el evaluador actualizado."
    )
...
for i, (model, color) in enumerate(zip(models, colors)):
    ...
    bars = ax.bar(x + offset, vals, width=bar_width * 0.92,
...
ax.set_title(
    "Métricas finales por modelo (promedio global sobre todas las técnicas)",
    fontsize=12, pad=12
)
...
plt.tight_layout()
out = OUTPUT_DIR / "barras_metricas_finales.png"
