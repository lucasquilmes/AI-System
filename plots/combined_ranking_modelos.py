"""
Combined — ranking de modelos en ambos datasets (side-by-side).
...
def model_means(dataset_name: str) -> pd.DataFrame:
    df = load_dataset(dataset_name)
    means = df.groupby("modelo")[list(METRICS.keys())].mean()
...
means_a = model_means(ds_a)
means_b = model_means(ds_b)

# Unión de modelos presentes en ambos
all_models = sorted(set(means_a.index) | set(means_b.index))
means_a = means_a.reindex(all_models)
means_b = means_b.reindex(all_models)

rank_a = rank_matrix(means_a)
rank_b = rank_matrix(means_b)

n_models  = len(all_models)
n_metrics = len(METRICS)
cmap = mcolors.LinearSegmentedColormap.from_list("rg", ["#2ecc71", "#f9f9a0", "#e74c3c"])

fig, axes = plt.subplots(1, 2, figsize=(20, 0.7 * n_models + 2.5),
...
fig.suptitle(
    "Ranking de modelos por dataset y métrica\n(verde = mejor · rojo = peor)",
    fontsize=13, y=1.01
)

out = OUTPUT_DIR / "combined_ranking_modelos.png"
